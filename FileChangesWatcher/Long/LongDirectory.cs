...
namespace SharepointSync {
    public class LongDirectory {
        ...
        public static string GetDirectoryName(string _path) {
            string file_name = Path.GetFileName(_path);
            string path = _path.Substring(0, _path.Length-file_name.Length);
            while(path.EndsWith(@"\") == true && path.Length>0) {
