...
namespace FileChangesWatcher
{
    ...
    // https://searchcode.com/codesearch/view/58086517/
    class AuditPolicy
    {
        // https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms721916(v=vs.85).aspx
        ...
        // https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms721903(v=vs.85).aspx
        ...
        public static Dictionary<AuditEventPolicy, AuditEventStatus> GetPolicies()
        {
            ...
            try
            {
                ...
                for (UInt32 eventType = 0; eventType < info.MaximumAuditEventCount; eventType++)
                {
                    ...
                    {
                        IntPtr itemPtr = new IntPtr(info.EventAuditingOptions.ToInt64() + (Int64)eventType * (Int64)Marshal.SizeOf(typeof(UInt32)));
                        UInt32 status = 0;
            ...
            try
            {
                ...
                for (UInt32 eventType = 0; eventType < info.MaximumAuditEventCount; eventType++)
                {
                    ...
                    for (UInt32 subcategoryIndex = 0; subcategoryIndex < subcategoryCount; subcategoryIndex++)
                    {
                        ...
                        if(AuditPolicySubcategories.ContainsKey(policyInfo.AuditSubCategoryGuid.ToString()) == true)
                        {
                            dict.Add(AuditPolicySubcategories[policyInfo.AuditSubCategoryGuid.ToString()], (AuditEventStatus)policyInfo.AuditingInformation);
                        }
                        else
                        {
                            Console.Write("Key " + policyInfo.AuditSubCategoryGuid.ToString() + " not found.");
                        }
                    }
